...
# approach: compute reverse human fashion, check if beautiful
# memory: O(1)
# runtime: O(n), where n is the total number of digits of all days 
def beautifulDays(i, j, k):
    beautifulDaysCount = 0
    for day in range(i, j + 1):
        reverse = int(''.join(list(reversed(str(day)))))
        if (day - reverse) % k == 0:
